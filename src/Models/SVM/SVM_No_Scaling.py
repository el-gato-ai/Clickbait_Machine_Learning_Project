...
def objective(trial, X_tr, y_tr, X_v, y_v):
    """
    Objective function για το SVM (ΧΩΡΙΣ SCALING).
    """
    ...
    params = {
        ...
        "probability": True,
        "random_state": 42,
        "max_iter": 2000  # Όριο για να μην κολλήσει, ειδικά χωρίς scaler
    }
    ...
    mlflow_helper.log_optuna_trial(trial, params, metrics, model, run_name_prefix="SVM_Trial")
...
def run_experiment():
    ...
    with mlflow.start_run(run_name="🔍_SVM_Hyperparameter_Tuning") as tuning_run:
        ...
        # 10 trials γιατί το SVM είναι αργό
    ...
    with mlflow.start_run(run_name="👑_SVM_Champion_Model") as final_run:
        best_params = study.best_params
        best_params.update({
            "probability": True,
            "random_state": 42,
            "max_iter": -1  # Αφήνουμε να τρέξει μέχρι τέλους
        })
