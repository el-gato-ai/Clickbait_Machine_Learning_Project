...
- `greek_news.xlsx` is maintained by the `data_collection` LangChain + Tavily workflow (run via `python -m data_collection`), which queries major Greek outlets per topic, summarizes each article in English, and appends deduplicated rows (URL, title, summary, topic) to the workbook.
...
| Custom Greek News Agent Pull | `raw/custom/greek_news.xlsx` | Collected via `python -m data_collection` |
